...
merge_fingers.py
-----------------
5개 손가락 STL 파일을 S3에서 받아와서, 30도 기울기와 손가락간 간격을 적용한 뒤
하나의 3MF 파일로 병합한다 (Bambu Studio / Orca Slicer가 여전히 손가락별로
개별 오브젝트로 인식할 수 있도록 오브젝트는 5개 그대로 유지).
...
프로그램에서 직접 호출할 때는 merge_hand()를 임포트해서 쓰면 된다
(server.py 같은 FastAPI 래퍼에서 이 함수를 그대로 호출하게 될 예정).
"""
...
TILT_DEG = -40.0        # 음수 = 반대 방향으로 기울임 (원래 30도가 반대 방향이어서 부호 반전 + 각도 변경)
...
def _build_scene(stl_paths: list[str], hand_label: str = "", y_offset: float = 0.0, scene: trimesh.Scene | None = None) -> trimesh.Scene:
    """
    thumb -> pinky 순서로 정렬된 STL 경로 리스트를 받아서,
    기울이고 정렬한 뒤 하나의 Scene으로 합친다.
    (원본 merge_fingers.py의 build_scene()과 동일한 회전/정렬 로직 — 검증된 부분은 그대로 유지)
    ...
    for i, (finger, path) in enumerate(zip(FINGER_ORDER, stl_paths)):
        mesh = _load_finger(path)
...
def merge_hand(userid: str, session: str, hand: str, shapes: dict, output_dir: str | None = None) -> str:
    """
    한 손(5손가락) STL을 S3에서 받아와 병합한 3MF를 만든다.

    shapes: {"thumb": "round", "index": "round", ...} — 손가락별 쉐입.
    output_dir: 결과 3MF를 저장할 폴더. 안 주면 printer/output/{userid}/{session}/{hand}/ 사용.

    반환값: 생성된 3MF 파일의 로컬 경로.
    """
    ...
    print(f"\n[Merge] {userid}/{session}/{hand} 의 STL 5개 다운로드 중...")
    stl_paths = download_finger_stls(userid, session, hand, shapes, stl_download_dir)

    print(f"[Merge] 병합 중 (기울기 {TILT_DEG}°, 간격 {SPACING_MM}mm)...")
    scene = _build_scene(stl_paths)
...
def merge_both_hands(userid: str, left_session: str, right_session: str,
    ...
    """
    양손(10손가락) STL을 S3에서 받아와 하나의 3MF로 병합한다.
    왼손은 Y=0 줄에, 오른손은 그보다 ROW_SPACING_MM만큼 떨어진 줄에 배치해서
    두 손이 겹치지 않게 한다. (한 손 안에서의 손가락 정렬 로직은 merge_hand와 동일)

    [중요] 왼손/오른손은 서로 다른 스캔 세션(scanId)을 갖는다 (HandScan 엔티티 =
    손 하나 = scanId 하나 구조). 그래서 session을 하나만 받지 않고 left_session/
    right_session을 따로 받는다.
    ...
    print(f"\n[Merge] {userid}/{left_session} 왼손 STL 5개 다운로드 중...")
    left_paths = download_finger_stls(userid, left_session, "left", left_shapes, left_dir)

    print(f"[Merge] {userid}/{right_session} 오른손 STL 5개 다운로드 중...")
    right_paths = download_finger_stls(userid, right_session, "right", right_shapes, right_dir)
...
def _parse_args():
    p = argparse.ArgumentParser(description="5손가락 STL을 하나의 3MF로 병합 (또는 --both로 양손 10손가락)")
    p.add_argument("--userid", required=True)
    p.add_argument("--session", default=None,
                    help="한 손만 병합할 때 그 손의 scanId")
    ...
    p.add_argument("--hand", choices=["left", "right"],
                    help="한 손만 병합할 때 지정 (--both와는 같이 안 씀)")
    p.add_argument("--both", action="store_true",
                    help="양손(10손가락)을 한 3MF로 병합")
    p.add_argument("--shape", default=None,
                    help="다섯 손가락 모두 같은 쉐입을 쓸 때 (예: round). "
                         "손가락별로 다르게 주려면 --{finger}-shape 옵션을 개별로 쓰세요.")
