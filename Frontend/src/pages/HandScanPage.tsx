...
import {
    buildScanSessions,
    formatMetricCurve,
    isFullyAnalyzedSession, representativePersonalColor,
    type ScanSession,
...
export function HandScanPage() {
  ...
  if (gateStatus === 'show' && latestCompletedSession) {
    ...
    const shapeLabel = session.recommendedShape ? getNailShape(session.recommendedShape)?.labelKo ?? session.recommendedShape : null
    const metricsLine = [
