...
//   return Array.from({ length: 3 }, (_, idx) => RESULT_IMAGES[(start + idx) % RESULT_IMAGES.length])   // 4 → 3
// }
...
//   const [designSaved, setDesignSaved] = useState(false)  // 추가
...
//         <h1>네일 디자인 생성 결과</h1>
//         <p>디자인을 선택해 3D 미리보기로 확인하고, 디자인을 저장하세요.</p>
...
//         <button
//           type="button"
//           className={`design-result-save ${designSaved ? 'is-saved' : ''}`}
//           onClick={handleSaveDesigns}
//           disabled={designSaved}
//         >
//           {designSaved ? '저장 완료 ✓' : '디자인 저장하기'}
//         </button>
//         {designSaved && (
...
export function NailDesignResultPage() {
  ...
  const [favoriteUrls, setFavoriteUrls] = useState<string[]>(() => getFavorites().map((f) => f.imageUrl))
  const [generationKey, setGenerationKey] = useState(0)
  const savedStateKey = `naily_design_saved:${prompt}:${generationKey}`
  const [designSaved, setDesignSaved] = useState<boolean>(() => {
    try { return localStorage.getItem(`naily_design_saved:${prompt}:0`) === 'true' } catch { return false }
  })
  ...
  const handleRegenerate = () => {
    ...
    setApiImages(null)
    setApiError(null)
    try { localStorage.removeItem(savedStateKey) } catch { /* noop */ }
  ...
  const handleSaveDesigns = () => {
    ...
    try { localStorage.setItem(savedStateKey, 'true') } catch { /* noop */ }
    setDesignSaved(true)
  }
