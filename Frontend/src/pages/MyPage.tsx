...
//   const [favorites, setFavorites] = useState<FavoriteItem[]>(getFavorites)
//   const [scanRecords, setScanRecords] = useState<HandScanRecord[]>(getHandScanRecords)

//   const refreshLists = () => {
//     setDesigns(getSavedDesigns())
//     setNailTipPrints(getNailTipPrintOrders())
//     setFavorites(getFavorites())
//     setScanRecords(getHandScanRecords())
//   }
...
//         <button
//           type="button"
//           className={tab === 'scan' ? 'is-active' : ''}
//           onClick={() => setTab('scan')}
//         >
//           손 분석 기록 ({scanRecords.length})
//         </button>
//       </nav>
...
//             <p className="mypage-v2__empty">손 분석 기록이 없습니다.</p>
//           ) : (
//             scanRecords.map((record) => (
//               <article key={record.id} className="mypage-v2__scan-row">
//                 <div className="mypage-v2__scan-swatch" style={{ background: record.skinToneHex }} />
//                 <div className="mypage-v2__scan-body">
//                   <p className="mypage-v2__scan-season">{record.seasonNameKo}</p>
//                   <p className="mypage-v2__scan-metrics">
//                     길이 {record.length.value}mm · 너비 {record.width.value}mm · 곡률 {record.cCurve.value}
//                   </p>
//                   <p className="mypage-v2__scan-shape">추천 쉐입: {record.recommendedShape}</p>
//                   <div className="mypage-v2__scan-palette">
//                     {record.personalColorPalette.slice(0, 5).map((hex) => (
//                       <span key={hex} className="mypage-v2__scan-chip" style={{ background: hex }} title={hex} />
//                     ))}
//                   </div>
//                   <p className="mypage-v2__scan-date">{new Date(record.capturedAt).toLocaleString('ko-KR')}</p>
//                 </div>
//               </article>
//             ))
//           )}
        ...
        <section className="mypage-v2__list">
          {scanRecords.length === 0 ? (
            ...
            scanRecords.map((record) => (
              <article
                key={record.id}
                className="mypage-v2__scan-row mypage-v2__scan-row--clickable"
                onClick={() => navigate('/scan/result', { state: { record, fromMypage: true } })}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => e.key === 'Enter' && navigate('/scan/result', { state: { record, fromMypage: true } })}
              >
                <div className="mypage-v2__scan-swatch" style={{ background: record.skinToneHex }} />
                <div className="mypage-v2__scan-body">
                  <p className="mypage-v2__scan-season">{record.seasonNameKo}</p>
