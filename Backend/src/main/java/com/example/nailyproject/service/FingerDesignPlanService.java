    ...
    private static final String SYSTEM_PROMPT = """
       ...
       [참고 이미지 색상 규칙 - 매우 중요]
        ...
        - design_type은 자유 텍스트입니다 (예: "glitter", "french tip", "matte", "gradient").
            ...
            [특정 글자/텍스트 요청 - 매우 중요]
                    확정된 입력 정보에 사용자가 네일에 넣고 싶어하는 특정 글자·이니셜·단어가
                    있다면(예: "이니셜 E 넣어줘", "LOVE라는 글자 넣어줘"), 그 글자를 앞뒤로
                    큰따옴표 하나씩만 붙여서 해당 손가락의 **parts 배열에만** 넣으세요.
                    예를 들어 E라는 글자를 원하면, parts에 이런 태그 하나를 추가하세요:
                    hand-painted lettering "E"
                    (참고용 설명일 뿐 그대로 베끼지 말고, 실제로는 그 글자 앞뒤에 큰따옴표
                    문자(")를 정확히 하나씩만 붙이세요.)
              - [매우 중요] design_type 필드(베이스 색/마감을 설명하는 문구)에는 이 글자를
                      다시 언급하지 마세요. design_type과 parts에 같은 내용을 중복해서 쓰면
                      최종 문장에서 "hand painted lettering "E" ... with hand painted lettering "E""
                      처럼 같은 표현이 두 번 겹쳐 나오게 됩니다. 글자 관련 내용은 parts에만 한 번
                      쓰고, design_type은 순수하게 베이스 색상/광택/질감만 설명하세요.
                    - 큰따옴표 안의 글자는 사용자가 요청한 문자 그대로 정확히 써야 합니다 (의역/변형 금지).
              - [매우 중요] 절대로 백슬래시(\\\\) 문자를 쓰지 마세요. 따옴표는 큰따옴표(") 문자
                      하나만 쓰고, 그 앞에 백슬래시를 붙이지 마세요. 올바른 예: "E" / 잘못된 예: \\\\"E\\\\"
                    - 이 요청이 없으면 큰따옴표를 쓰지 마세요 (다른 목적으로 큰따옴표를 사용하지 마세요 —
                      이후 단계에서 "큰따옴표가 있으면 텍스트 요청이 있는 것"으로 판단하기 때문입니다).
              - [매우 중요] 이 글자 기능은 사용자가 "글자를 넣어줘", "이니셜 넣어줘", "단어를
                      새겨줘"처럼 명시적으로 텍스트/글자를 요청했을 때만 쓰세요. 동물·사물·도형
                      모티프(예: 거북이, 하트, 별, 리본)를 그 이니셜 글자로 대체해서 표현하지
                      마세요 — "turtle"을 요청했다고 "T"라는 글자를 넣는 식으로 창의적으로
                      바꾸면 안 됩니다. 그런 모티프는 실제 형태를 반영한 참/아트로 표현하세요
                      (예: "3D turtle-shaped charm", "turtle silhouette line art").
